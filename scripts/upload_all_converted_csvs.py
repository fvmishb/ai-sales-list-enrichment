#!/usr/bin/env python3
"""
変換されたCSVファイルを全てBigQueryにアップロードするスクリプト
"""
...
def upload_csv_to_bigquery(csv_file, table_name="companies.raw"):
    ...
    try:
        ...
        if result.returncode == 0 or "Upload complete" in result.stdout or "DONE" in result.stdout or "Waiting on" in result.stdout:
