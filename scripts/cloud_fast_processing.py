...
async def process_industry_cloud(industry: str, limit: int = None, max_workers: int = 20):
    """Cloud Run用業界別高速処理"""
    logger.info(f"Starting cloud fast processing for industry: {industry}")
...
async def main():
    parser = argparse.ArgumentParser(description='Cloud Run fast batch processing for company data')
    ...
    try:
        await process_industry_cloud(args.industry, args.limit, args.workers)
